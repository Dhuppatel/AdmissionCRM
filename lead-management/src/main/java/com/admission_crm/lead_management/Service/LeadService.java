...
public class LeadService {
    ...
    private final InstitutionQueueService institutionQueueService;
    ...
    public Lead createLead(LeadRequest leadRequest, String userEmail) {
        ...
        Double score = leadScoringService.calculateLeadScore(lead);
        lead.setLeadScore(score);

        Lead savedLead = leadRepository.save(lead);

        institutionQueueService.addToQueue(savedLead);
    ...
    private void freeCounselorSlot(String counselorId) {
        userRepository.findById(counselorId).ifPresent(counselor -> logAudit("SYSTEM", "COUNSELOR_FREED", counselorId, "Counselor",
    ...
    private void tryAutoAssignmentForCounselor(String counselorId, String institutionId, String userEmail) {
        ...
        Lead nextLead = institutionQueueService.getNextLeadFromQueue(institutionId);
        if (nextLead != null) {
    ...
    public void deleteLead(String leadId, String userEmail) {
        ...
        if (lead.getStatus() == LeadStatus.QUEUED) {
            institutionQueueService.removeFromQueue(leadId);
        }
    ...
    public List<LeadQueueInfo> getQueueStatus(String institutionId) {
        return institutionQueueService.getQueueStatus(institutionId);
    }

    // Get queue size for an institution
    public int getQueueSize(String institutionId) {
        return institutionQueueService.getQueueSize(institutionId);
    }
...
//        }
    ...
    public Lead updateLeadPriority(String leadId, Lead.LeadPriority newPriority, String userEmail) {
        ...
        Double newScore = leadScoringService.calculateLeadScore(lead);
        ...
        if (lead.getStatus() == LeadStatus.QUEUED) {
            ...
            if (institution != null) {
                ...
                institutionQueueService.removeFromQueue(leadId);
                institutionQueueService.addToQueue(updatedLead);
            }
        }
    ...
    public Map<Integer, String> getQueueWaitingTimeEstimates(String institutionId) {
        List<LeadQueueInfo> queueInfo = institutionQueueService.getQueueStatus(institutionId);
